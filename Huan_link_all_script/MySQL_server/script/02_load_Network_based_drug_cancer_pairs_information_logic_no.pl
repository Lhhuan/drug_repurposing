...
my %hash1;
my $f1 = "/f/mulinlab/huan/ALL_result_ICGC_ALL_drug/network/random_walk_restart/output/16_judge_the_shortest_drug_target_cancer_gene_no_logic.txt.gz";
...
while (<$I1>) {
    ...
    unless(/^the_shortest_path/){
        ...
        $k="$k\t$drug_entrze\t$drug_ENSG\t$Drug_type\t$drug_target_score\t$drug_target_network_id\t$the_final_logic";
            ...
            $sth = $dbh -> prepare("insert into Network_based_drug_cancer_pairs_information_logic_no (the_shortest_path,path_logic,path_length,drug_name_network,
