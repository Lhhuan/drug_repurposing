...
while (<$I>) {
    ...
    unless(/^Drug_chembl_id_Drug_claim_primary_name/){
        ...
        $Drug_chembl_id =~ s/NONE|NULL/NA/g;
        my $k = "$Drug_chembl_id_Drug_claim_primary_name\t$Drug_chembl_id";
        unless(exists $hash1{$k}){ #去重
            $hash1{$k} =1;
            if ($Drug_chembl_id eq "NA") { #这些值在mysql要按照空处理。在表里填NULL
                undef($Drug_chembl_id) 
            }

        $sth = $dbh -> prepare("insert into Drug_chembl_id (Unique_drug_name,Drug_chembl_id) 
                                values (?, ?)");
        $sth -> execute($Drug_chembl_id_Drug_claim_primary_name,$Drug_chembl_id); #开始插
        }
    }
}
$sth -> finish(); #插入完
