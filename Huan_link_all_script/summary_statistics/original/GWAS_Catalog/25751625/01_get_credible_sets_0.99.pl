...
# while(<$I1>)
{
    ...
    unless($line=~/^File_name_Downloaded/){
        ...
        if($PMID=~/25751625/){
            chdir "/f/mulinlab/huan/summary_statistics/original";
            chdir "${Folder_name_local}/PAINTOR/fine_mapping_output_result";
            # chdir "$PMID/test";
            ...
            $header = "$header\tstandard_error\(SE\)\todds_ratio\(OR\)\tOR_95I\tZscore\tTstat\tChisq\tOR_P\tBeta_P\tT_P\tsample_size\tAC\tytx\tPosterior_Prob\tone_file_end\t25751625";
            print $O1 "$header\n";
