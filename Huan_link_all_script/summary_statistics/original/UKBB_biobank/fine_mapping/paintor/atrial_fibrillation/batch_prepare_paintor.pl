...
my $f1 ="/f/mulinlab/huan/summary_statistics/original/atrial_fibrillation_final.txt";
open my $I1, '<', $f1 or die "$0 : failed to open input file '$f1' : $!\n";
my %hash1;
my $pm = Parallel::ForkManager->new(3); ## 设置最大的线程数目
...
# while(<$I1>)
{
    ...
    if($Folder_name_local=~/UKBB_biobank/){
        ...
        $ENV{'input_path'} = $PMID ;
        system "bash paintor_finemapping0605_revise_huan_ukbb.sh"; #运行准备文件
        chdir "./$PMID/";#切换目录
        ...
        system "bash ../run_paintor.sh"; #准备文件完成，开始运行paintor
