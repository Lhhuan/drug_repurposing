...
my $f1 ="./17660530_Multiple_sclerosis.txt";
my $fo1 ="./17660530_Multiple_sclerosis_normalized.txt";
...
while(<$I1>) 
{
    ...
    my @f= split /\t/;
     if(/^rs/){
         ...
         my $alt_allele = $f[8];
         my $odds_ratio = $f[14];
         print $O1 "$chr\t$pos\t$rsid\t$ref_allele\tNA\t$alt_allele\tNA\tNA\tNA\tNA\tNA\tNA\tNA\t$odds_ratio\tNA\tNA\tNA\tNA\t$OR_P\tNA\tNA\t12360\tNA\tNA\n";
     }
}
