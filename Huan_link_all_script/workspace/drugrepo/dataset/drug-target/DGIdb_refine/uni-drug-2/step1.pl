...
select $O;
print"drug_chembl_id|drug_claim_name\tdrug_stage\tinteraction_types\tinteraction_claim_source\n";
...
while(<$I>)
{
   ...
   unless(/^gene_name/){
       ...
       if($f[8] =~ /NULL|NONE/){  
           ...
           my $key1 = "$drug_claim_name\t$drug_stage\t$interaction_types\t$interaction_claim_source";
          ...
          my $interaction_claim_source = $f[3];
           my $interaction_types = $f[4];
           my $drug_stage = "unknown";
           my $key2 = "$drug_chembl_id\t$drug_stage\t$interaction_types\t$interaction_claim_source";
...
while(<$I1>)
{
   ...
   unless(/^ENSGID/){
       ...
       my $drug_stage = $f[8];
       if($f[16] !~ /NA/){
          my $key3 = "$drug_chembl_id\t$drug_stage\t$interaction_types\t$drug_source";
          unless(exists $hash3{$key3}){
           ...
           my $key4 = "$drug_name\t$drug_stage\t$interaction_types\t$drug_source";
