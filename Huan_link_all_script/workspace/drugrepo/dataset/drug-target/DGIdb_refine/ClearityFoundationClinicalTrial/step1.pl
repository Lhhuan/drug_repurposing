...
my $fo = "./interactions_v3-ClearityFoundationClinicalTrial.txt";
...
#将interactions_v3.tsv中的源于ClearityFoundationClinicalTrial的数据筛选出来。
while(<$I>)
{
   chomp;
   my @f = split/\t/;
   my $source = $f[3];
   if ($f[3] =~ /ClearityFoundationClinicalTrial/){
       print $_."\n"
   }
}
