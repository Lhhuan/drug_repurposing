...
select $O1;
print"drug_chembl_id\tdrug_stage\tinteraction_type\tinteraction_claim_source\tdrug_claim_name\tdrug_indication\tdrug_indication_source\n";
select $O2;
print"drug_claim_name\tdrug_stage\tinteraction_type\tinteraction_claim_source\tdrug_chembl_id\tdrug_indication\tdrug_indication_source\n";
select $O3;
...
while(<$I2>)
{
 ...
 foreach my $drug_chembl_id(sort keys %hash1){
       if(exists $hash2{$drug_chembl_id}){ 
        ...
        elsif(exists $hash3{$drug_chembl_id}){ #没有chembl_id或者用chembl_id匹配不成功的用drug_claim_name匹配。
             my @t = @{$hash1{$drug_chembl_id}};
             my @v3 = @{$hash3{$drug_chembl_id}};
             foreach my $t(@t){
                foreach my $v3(@v3){
                    my $k5 = "$drug_chembl_id\t$t\t$v3";
