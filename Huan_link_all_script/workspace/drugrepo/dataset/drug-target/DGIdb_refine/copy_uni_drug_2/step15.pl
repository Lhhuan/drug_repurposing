...
my $f1 ="./step14_chembl_unexist_phase_0.txt";
my $f2 = "./interactions_v3.tsv";
my $f3 = "./clinical.trial-drug-indication.txt";  
my $fo1 = "./step15_drug-indication-exist.txt"; #在clinical.trial中存在适应症的药物
my $fo2 = "./step15_drug_no_indication.txt"; #在clinical.trials中不存在适应症的药物
my $fo3 = "./step15_no_drug-name.txt";#在interactions_v3.tsv中不中存在的chembl_id(如果数据处理正确，这个文件中数据为空)
...
select $O1;
print"drug_chembl_id\tdrug_stage\tinteraction_type\tinteraction_claim_source\tmax_phase\tfirst_approval\tindication_class\tdrug_claim_name\tdrug_indication\tphase\tNCDid\tdrug_indication_source\n";
select $O2;
print"drug_chembl_id\tdrug_stage\tinteraction_type\tinteraction_claim_source\tmax_phase\tfirst_approval\tindication_class\tdrug_claim_name\n";
select $O3;
...
while(<$I2>)
{
   ...
   unless(/^gene_name/){
       ...
       $hash2{$drug_chembl_id}=$drug_claim_name;
       }
}
...
while(<$I3>)
{
 ...
 foreach my $drug_chembl_id(sort keys %hash1){
       if(exists $hash2{$drug_chembl_id}){ 
            ...
            my $drug_claim_name = $hash2{$drug_chembl_id};
            if(exists $hash3{$drug_claim_name}){
                my @v3 = @{$hash3{$drug_claim_name}};
                foreach my $v3(@v3){
                    my $k4 = "$drug_chembl_id\t$v1\t$drug_claim_name\t$v3";
                ...
                my $v1 = $hash1{$drug_chembl_id};
                my $drug_claim_name = $hash2{$drug_chembl_id};
                my $k5 = "$drug_chembl_id\t$v1\t$drug_claim_name";
