...
my $f1 ="./step14_chembl_unexist_phase=0.txt";
my $f2 = "./interactions_v3.tsv";
my $f3 = "./clinical.trial-drug-indication.txt";  
my $fo1 = "./step14-1_drug-indication-exist.txt"; #在clinical.trial中存在适应症的药物
my $fo2 = "./step14-1_drug_no_indication.txt"; #在clinical.trials中不存在适应症的药物
my $fo3 = "./step14-1_no_drug-name.txt";#在interactions_v3.tsv中不中存在的chembl_id(如果数据处理正确，这个文件中数据为空)
...
select $O1;
print"drug_chembl_id\tdrug_stage\tinteraction_type\tinteraction_claim_source\tmax_phase\tfirst_approval\tindication_class\tdrug_claim_primary_name\tdrug_indication\tphase\tNCDid\tdrug_indication_source\n";
select $O2;
...
while(<$I2>)
{
   ...
   unless(/^gene_name/){
       ...
       $hash2{$drug_chembl_id}=$drug_claim_primary_name;
       }
}
