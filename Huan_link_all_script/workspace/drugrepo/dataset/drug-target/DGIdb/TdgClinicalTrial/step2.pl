...
my $fi_drug ="./interactions_v3-TdgClinicalTrial.txt";
my $fi_indication = "./pa54_rask-andersen_suptable1-LING.txt";
my $fi_drug_indication = "./step2_result_drug_indication";
my $fi_unmatch = "./step2_unmatch_drug";
...
while(<$I2>)
{
   ...
   unless(/^INDEX/){
        my @f = split/\t/;
        my ($drug_Name, $Indication, $Year_of_Approval) = ($f[2], $f[3], $f[5]);
        my $k = join"\t",@f[3,5];
