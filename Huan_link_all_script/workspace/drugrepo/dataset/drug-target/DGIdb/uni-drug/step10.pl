...
select $O1;
print"drug_claim_name\tinteraction_claim_source\tdrug_substance_name\tdisease_concept_name\tdisease_full_name\tdrug_indication_source\n";
select $O2;
print"drug_claim_name\tinteraction_claim_source\tdrug_substance_name\tdisease_concept_name\tdisease_full_name\tdrug_indication_source\n";
select $O3;
       ...
       if(exists $hash2{$drug_claim_name}){ #以$drug_active_name为key进行匹配
            ...
            my @v2 = @{$hash2{$drug_claim_name}};
             foreach my $interaction_claim_source(@interaction_claim_source){
                foreach my $v2(@v2){
                    my $k4 = "$drug_claim_name\t$interaction_claim_source\t$v2";
            ...
            my @v3 = @{$hash3{$drug_claim_name}};
             foreach my $interaction_claim_source(@interaction_claim_source){
                foreach my $v3(@v3){
                    my $k5 = "$drug_claim_name\t$interaction_claim_source\t$v3";
            ...
            my @interaction_claim_source = @{$hash1{$drug_claim_name}};
            foreach my $interaction_claim_source(@interaction_claim_source){
                my $k6 = "$drug_claim_name\t$interaction_claim_source";
