...
my $fi_step2result ="./step2-result-drug-action";
my $fi_p_e ="./wgEncodeGencodeUniProtV24lift37.txt";
my $fi_e_e ="./ensGene.txt";


open my $fh_step2result, '<', $fi_step2result or die "$0 : failed to open input file '$fi_step2result' : $!\n";
...
while(<$fh_step2result>)
{
    ...
    my $uniprot_ID = $f1[6];
   # my $drug = $f1[4];
    my $t = join "\t", @f1[0..5,7..11];
  #  my $t = join "\t", @f1[0,1,2,3,4,5,7..12];
