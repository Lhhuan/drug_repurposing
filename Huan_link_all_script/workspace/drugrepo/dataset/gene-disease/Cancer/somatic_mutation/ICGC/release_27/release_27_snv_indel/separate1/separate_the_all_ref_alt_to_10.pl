...
system "less ../04_out_gene_varint_enhancer_target_info.vcf |wc -l > line.txt";
my $f1 ="line.txt";
...
while(<$I1>)
{
   ...
   system "sed -n '$end_final,${line}p' ../04_out_gene_varint_enhancer_target_info.vcf > ./04_out_gene_varint_enhancer_target_infofinal.vcf" ;
    for (my $i = 0;$i<9;$i=$i+1){ #进行39个循环
        ...
        system "sed -n '$start,${end}p' ../04_out_gene_varint_enhancer_target_info.vcf > ./04_out_gene_varint_enhancer_target_info${i}.vcf" ;  #取特定行到特定文件
    }
}
