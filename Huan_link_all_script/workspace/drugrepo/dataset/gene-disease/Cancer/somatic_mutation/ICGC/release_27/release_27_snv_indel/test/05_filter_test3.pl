 #把04_out_gene_varint_enhancer_target_info_vep.vcf可以map到gene的文件筛选得临近基因法的第一个层面文件05_varint_gene_in_level3_1.vcf，得临近基因法的第二个层面文件05_varint_gene_in_level3_2.vcf,得没有map到protein coding 的mutation文件05_varint_out_gene_in_level3_3.vcf
#!/usr/bin/perl
use warnings;
use strict; 
use utf8;
...
my $fo3 = "./05_varint_out_gene_in_level3_3.vcf";
                ...
                if($i=~/^NEAREST/){
                    ...
                    foreach my $NEAREST(@NEARESTs){
                        ...
                        print $O2 "$output\tlevel3.2\n";
                            ...
                            unless(exists $hash6{$variation_id}){  #判断variant没有在上面level3.2出现过。
