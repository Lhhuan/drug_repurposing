            ...
            if ($i =~/BIOTYPE/){
                 ...
                 if ($f3[1]=~/protein_coding/){ #BIOTYPE必须是protein_coding
                     if ($consequence =~/$level1_1/){ #consequence只要有$level1_1中的任意一个出现，都是level1_1
                        $hash1{$variation_id} =1;  #进入level1-1的不再进入其他variant map 到gene的规则，所以此处为去重做准备
...
foreach my $ID(sort keys %hash2 ){
    unless(exists $hash1{$ID}){  #只要variant 的一个注释落在level1.1，就不再进入level1.2。
        my @vs =  @{$hash2{$ID}};
        foreach my $v(@vs){
            my @f =split/\s+/,$v;
            my $consequence = $f[6];
            ...
            foreach my $i(@f2){
                if ($i =~/BIOTYPE/){
                    my @f3 = split/\=/,$i;
                    if ($f3[1]=~/protein_coding/){ #BIOTYPE必须是protein_coding
                        if ($consequence =~/$level1_2/){ #consequence只要有$level1_2中的任意一个出现，都是level1_2
                            $hash3{$variation_id} =1;  #进入level1-2的不再进入其他variant map 到gene的规则，所以此处为去重做准备
                            print $O1 "$v\tLevel1_2_protein_coding\n";  
                            # print STDERR "$consequence\n";
                        }
                        else{
                            push @{$hash4{$variation_id}},$v;
                        }
                    }
                    else{
                        push @{$hash4{$variation_id}},$v;
                    ...
                    unless(exists $hash3{$variation_id}){
                        push @{$hash4{$variation_id}},$v; 
                    }
                }
            }
...
foreach my $ID(sort keys %hash4 ){
    unless(exists $hash3{$ID}){  #只要variant 的一个注释落在protein coding，就算这个variant 所多对应的gene是该protein coding的基因，而不再进入其他variant map 到gene的规则，所以此处为去重。
        my @vs =  @{$hash4{$ID}};
        foreach my $v(@vs){
