...
将文件Cosmic_all_coding_revel.txt中revel score大于0.5的数据筛选出来并去重得文件unique_largethan0.5_driver_mutation.txt，共167759行 。

cat Cosmic_all_coding_revel.txt | perl -ane '{next if($_ =~ /^chr/);@f = split/\t/;print "$f[0]\t$f[3]\n"}' | sort -u > unique_driver_mutation.txt
将文件Cosmic_all_coding_revel.txt中按$f[0]进行去重，得去重得文件unique_driver_mutation.txt，共763156。
...
将文件Cosmic_all_coding_no_revel_path.txt中按$f[0]进行去重，得去重得文件unique_no_revel_path.txt，共51368行。
