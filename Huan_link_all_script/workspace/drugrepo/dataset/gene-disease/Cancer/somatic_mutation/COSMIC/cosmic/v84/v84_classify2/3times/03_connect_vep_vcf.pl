...
my $f1 ="./CosmicCodingMuts_largethan2_nm_vep.vcf";
my $f2 ="./CosmicNonCodingVariants_largethan2_nm_vep.vcf";
my $f3 ="./All_cosmic_Muts_largethan2_nm.vcf";
...
my $title = "position\tref\talt\tID\tlocation\tENSG_ID\tENST_ID\tvariant_type\tsymbol\tCDS_position\tProtein_position\n";
select $O1;
print $title;
         ...
         unless($f[2] =~ /-/){  #不要insertion的
             unless($f[1] =~ /-/){  #不要deletion的
                 ...
                 foreach my $variant(@fv){
                     if ($ENSG_ID =~ /-/){
                         my $symbol = "NA";
                         my $header = "$location\t$ENSG_ID\t$ENST_ID\t$variant\t$symbol\t$CDS_position\t$Protein_position";
