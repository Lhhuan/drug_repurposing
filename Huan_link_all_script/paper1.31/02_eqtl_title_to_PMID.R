...
setwd ("~/paper/")
t <- read_xlsx("jou.xlsx",col_names = F)  #读杂志列表
ids <-  c(t$X__1)

result <- data.frame(title = character())    #建一个新表

for (id in ids){
  fi <- paste0(id,"_result",".xls")
...
setwd ("~/paper/01")
write.table(result1,'01_variant_title_to_PMID.txt',row.names = F, col.names = T)
write.table(no_match,'01_no_match_variant_title_to_PMID.txt',row.names = F, col.names = T)
