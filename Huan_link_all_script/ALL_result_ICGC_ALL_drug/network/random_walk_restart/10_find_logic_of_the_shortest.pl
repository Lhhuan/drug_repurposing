#用"/f/mulinlab/huan/All_result_ICGC/network/the_shortest_path/normal_network_num.txt"为./output/09_the_shortest_path.txt里面的路径寻找start和end的逻辑关系,得./output/10_start_end_logical.txt
#!/usr/bin/perl
use warnings;
use strict; 
use utf8;
...
my $f2 ="./output/09_the_shortest_path.txt";
my $fo1 ="./output/10_the_shortest_path_edge.txt"; #中间文件#这里是把每个start和end的最短路径的边存下来
my $fo2 ="./output/10_start_end_logical.txt"; 
...
select $O1;
print "start\tend\tthe_shortest_edge\tpath_length\n"; 
select $O2;
print "start\tend\tlogic_direction\tpath_length\n";
...
my $f3 ="./output/10_the_shortest_path_edge.txt";
...
foreach my $start_end (sort keys %hash2){
    ...
    if (exists $hash3{$start_end}){
        my $path_length = $hash3{$start_end};
        if($count_in==0 && $count_a==0){   #当-|和->都为0，即全部为—时，start对end没有任何激动和抑制作用
            print $O2 "$start_end\t-\t$path_length\n";
        }
        elsif($in>0){
            print $O2 "$start_end\tin\t$path_length\n"; #当-|为奇数时，无论->个数为多少，start对end都是抑制作用
        }
        else{
            print $O2 "$start_end\ta\t$path_length\n"; #当-|为偶数时，无论->个数为多少，start对end都是激动作用
        }
    }
}
