#利用记录drug target 数目的文件./output/9.21_drug_target_num.txt，把./output/huan_data_rwr/random_select/rwr_result_top_0.044/${num}/result${i}.txt 和./output/08_drug_start_comma_end.txt中的end 取交集。得文件./output/9.25_random_overlap_fact.txt
...
my $f1 ="./output/9.21_drug_target_num.txt";#输入的是drug target 数目
open my $I1, '<', $f1 or die "$0 : failed to open input file '$f1' : $!\n";
my $f2 ="./output/08_drug_start_comma_end.txt";#输入的是drug 及其rwr top0.044的end
open my $I2, '<', $f2 or die "$0 : failed to open input file '$f2' : $!\n";
my $fo1 ="./output/9.25_random_overlap_fact.txt"; 
    ...
    foreach my $drug(@drugs){
        if (exists $hash2{$drug}){
            ...
            for(my $i=1; $i<1001;$i++){# 把./huan_data_rwr/random_select/rwr_result_top_0.044/${num}/result${i}.txt 和08_drug_start_comma_end.txt中的end 取交集
                my $f3 ="./output/huan_data_rwr/random_select/rwr_result_top_0.044/${num}/result${i}.txt";
