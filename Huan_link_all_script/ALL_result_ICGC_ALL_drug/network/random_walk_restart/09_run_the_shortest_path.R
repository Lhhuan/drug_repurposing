...
org<-read.table("./output/network_gene_num.txt",header = T)
spe<-data.frame(org$id)#把所有的顶点数读进来。
input<-read.table("./output/normal_network_num.txt",header = T)  #把文件读进来并转为data.frame
...
pairs <-read.table("./output/08_uni_start_end_shortest.txt",header = F)%>%as.data.frame()#把要看最短路径start和end的列表读进来。
rs <- data.frame()#构造一个表
...
write.table(rs,"./output/09_the_shortest_path.txt",row.names = F, col.names = T,quote =F,sep="\t")#把表存下来
