...
while(<$I1>)
{
    chomp;
    my $num = $_;
    my $dir = "./output/huan_data_rwr/random_select/rwr_result_top_0.044/${num}";
    mkdir $dir unless -d $dir; #建文件夹
    for(my $i=1; $i<1001;$i++){#
        my $line = 0.044*12277;
        my $line2  = sprintf "%.f", $line; # 这个是四舍五入取整
        system "head -n $line2 ./output/huan_data_rwr/random_select/rwr_result/${num}/result${i}_sorted.txt > ./output/huan_data_rwr/random_select/rwr_result_top_0.044/${num}/result${i}.txt";
    }
}
