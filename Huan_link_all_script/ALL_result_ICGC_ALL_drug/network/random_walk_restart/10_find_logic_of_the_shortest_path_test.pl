#用"/f/mulinlab/huan/All_result_ICGC/network/the_shortest_path/normal_network_num.txt"为./output/09_the_shortest_path.txt里面的路径寻找start和end的逻辑关系,得./output/10_start_end_path_logical.txt
use warnings;
use strict; 
use utf8;
...
select $O1;
print "start_tend\tpath_logic_direction\n";
    ...
    my @f= split /\t/;
     unless(/^start/){
        ...
        if ($path_length>0 && $path_length <3.606){ #如果最短路径只有一个元素，说明没有最短路径，故此最短路径不保存下来,如果路径长度大于网络中平均最短路径长度，则说明这个路径太长了没有意义，所以只把路径长度大于0且小于平均路径3.606的最短路径留下来
            ...
            my $output = "${start}_${end}\t$path_logic_final";
            print $O1 "$output\n";
         }
     }
}
