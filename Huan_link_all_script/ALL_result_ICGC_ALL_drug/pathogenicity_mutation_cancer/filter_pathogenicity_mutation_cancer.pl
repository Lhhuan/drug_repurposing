...
my $f1 = "/f/mulinlab/huan/workspace/drugrepo/dataset/gene-disease/Cancer/somatic_mutation/ICGC/release_27/release_27_snv_indel/cadd_score/SNV_Indel_cadd_score_simple.txt";
...
foreach my $Mutation_ID (sort keys %hash1){
    ...
    if(exists $hash2{$Mutation_ID}){
            unless(exists $hash4{$Mutation_ID}){ #对mutation id进行去重,这里才是致病性的mutation，score >=15,并且cancers pecific occurance >=2
