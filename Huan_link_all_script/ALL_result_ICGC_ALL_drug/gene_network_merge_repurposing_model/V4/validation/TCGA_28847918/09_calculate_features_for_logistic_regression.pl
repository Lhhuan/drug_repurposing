...
open my $I2, '<', $f2 or die "$0 : failed to open input file '$f2' : $!\n";
my $f3 = "./output/06_sample_pathogenicity_mutation_number.txt";
...
$header = "$header\taverage_effective_drug_target_score\tmax_effective_drug_target_score\taverage_mutation_frequency\tmax_mutation_frequency";
...
print $O1 "$header\n";
my (%hash1,%hash2,%hash3,%hash4,%hash5,%hash6,%hash7,%hash8,%hash9,%hash10,%hash11,%hash12,%hash13,%hash30,%hash32,%hash33);
...
while(<$I2>)
{
    chomp;
    my @f= split/\t/;
    unless (/^paper_sample_name/){
        my @f =split/\t/;
        my $paper_sample_name = $f[0];
        my $score = $f[4];
        my $sv_or_cnv_type = $f[7];
        my $sv_or_cnv_id  =$f[6];
        my $oncotree_ID = $f[-2];
        my $oncotree_ID_type = $f[-1];
        #--------------------------------------------------------------把cnv push进hash 数组
        my $k = "$paper_sample_name\t$oncotree_ID\t$oncotree_ID_type";
        my $v = "$sv_or_cnv_id\t$score";
        # print STDERR "$v\n";
        push @{$hash12{$k}},$v;
    }
}
...
while(<$I3>)
{
    ...
    unless (/^Sample/){
        my @f =split/\t/;
        my $sample = $f[0];
        my $mutation_number = $f[1];
        $hash33{$sample}= $mutation_number;
    }
}
...
foreach my $k (sort keys %hash1){
    ...
    # print "$k\t@cancer_specific_mutation\n";
    my $averge_mutation_frequency = $sum_mutation_f/$mutation_num;   
    #---------------------------------------------------计算 average mutation frequency
    ...
    my @outputs = ();
    my $output1 = "$average_effective_drug_target_score\t$max_effective_drug_target_score\t$averge_mutation_frequency\t$max_mutation_frequency\t$averge_mutation_pathogenicity\t$max_mutation_pathogenicity";
    $output1 = "$output1\t$averge_mutation_map_to_gene_level_score\t$max_mutation_map_to_gene_level_score";
    $output1 = "$output1\t$average_path_length\t$min_path_length\t$min_rwr_normal_P_value\t$median_rwr_normal_P_value";
    ...
    my @ts = split/\t/,$k;
    my $sample = $ts[4];
    my $all_p_mutation_number = $hash33{$sample};
    my @mutation_ids = @{$hash32{$k}};
    my %hash34;
    @mutation_ids =grep {++$hash34{$_}<2} @mutation_ids;
    my $matching_mutation_number = @mutation_ids;
    my $matching_score = $matching_mutation_number / $all_p_mutation_number;
    # my $matching_score =0;
