#用"/f/mulinlab/huan/ALL_result_ICGC_ALL_drug/output/ICGC_occurthan1_snv_indel_project_oncotree_normalized.txt"中的detail oncotree term 和 oncotree term 和 ./output/13_indication_and_cancer_lable_info.txt
...
$header = "$header\tcancer_oncotree_id_type\tpredict\tpredict_value\tDrug_indication_Indication_class\tindication_OncoTree_detail_ID\tindication_OncoTree_main_ID\tcancer_lable";
print $O1 "$header\n";
...
while(<$I2>)
{
    ...
    unless(/^Drug_chembl_id_Drug_claim_primary_name/){
        ...
        if (exists $hash1{$k}){
            ...
            my $output = join("\t",@f[0..3],$cancer_oncotree_term,@f[4..10]);
