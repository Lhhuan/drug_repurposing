...
my $f2 = "./output/03_actionable_mutation_transvar_ref_alt.txt";
    ...
    unless(/^oncotree_term_detail/){
        ...
        if($hgvsg =~/\bNA\b/){
            ...
            if(exists $hash1{$hgvsg}){
                ...
                my $output3 = "$output\t$disease";
                unless (exists $hash6{$output3}){ #可以转成hgvs的actionable_mutation,但不在icgc的目标文件中文件
                    $hash6{$output3} =1;
                    print $O5 "$output3\n";
                }
            }
