#将./output/13_transvar_ref_alt.txt 和./output/082_merge_Drug_top_number_drug_sample_mutation_hgvsg_cancer_term.txt 通过mutation merge 到一起，得
...
my $fo1 = "./output/15_pancancer_pandrug_hit_action_mutation.txt";
open my $O1, '>', $fo1 or die "$0 : failed to open output file '$fo1' : $!\n";
my $fo2 = "./output/15_pancancer_pandrug_potential_mutation.txt";
open my $O2, '>', $fo2 or die "$0 : failed to open output file '$fo2' : $!\n";
my $fo3 = "./output/15_pancancer_pandrug_hit_actionable_or_not.txt";
...
print $O1 "$output\tfinal_actionable_mutation\n";
print $O2 "$output\n";
print $O3 "$output\tfinal_actionable_mutation\tHit_actionable_or_not\n";
...
while(<$I1>)
{
    ...
    unless(/^Drug_chembl_id_Drug_claim_primary_name/){
        ...
        push @{$hash1{$hgvsg}},$final_variant;
    }
}
