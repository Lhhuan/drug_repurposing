...
org1<-read.table("./output/33_pandrug_top_0_0.4_final.txt",header = T,sep = "\t") %>% as.data.frame()
org2<-read.table("./output/33_pandrug_top_0.4_1_final.txt",header = T,sep = "\t") %>% as.data.frame()
...
pdf("34_pandrug_top_0.4_1_group_comparison_p.pdf",height = 4,width = 3.8) #把图片存下来
...
my_comparisons <- list(c("0_0.4","0.4_1") ) 
...
#----------------violin
pdf("34_pandrug_top_0.4_1_group_comparison_violin.pdf",height = 4,width = 3.8) #把图片存下来
