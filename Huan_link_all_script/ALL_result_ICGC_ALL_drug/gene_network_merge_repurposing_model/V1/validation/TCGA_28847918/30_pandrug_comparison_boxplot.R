...
org1<-read.table("./output/29_pandrug_top_0_0.3_final.txt",header = T,sep = "\t") %>% as.data.frame()
org2<-read.table("./output/29_pandrug_top_0.3_1_final.txt",header = T,sep = "\t") %>% as.data.frame()
...
pdf("30_pandrug_top_0.3_1_group_comparison_p.pdf",height = 4,width = 3.8) #把图片存下来
...
my_comparisons <- list(c("0_0.3","0.3_1") ) 
...
#----------------violin
pdf("30_pandrug_top_0.3_1_group_comparison_violin.pdf",height = 4,width = 3.8) #把图片存下来
