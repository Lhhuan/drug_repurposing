...
library(caret)
library(lattice)
setwd("/f/mulinlab/huan/ALL_result_ICGC_ALL_drug/gene_network_merge_repurposing_model/V1/test_data/")
...
# pre_all <- glm(drug_repurposing ~.,family=binomial(link = "logit"),data = org1)
# summary(pre_all)
# step(pre_all)
#将org1数据分成随机十等分
...
for(i in 1:5){
  ...
  #----------------------------------------------
  true_value1 =fold_test[,17]
  #--------------------------把所有的被预测的测试数据的结果放到一个表里
  tmp<-data.frame(true_value1= fold_test[,17],predict_value1=fold_predict)
  rs <- bind_rows(rs,tmp)
}

#----------------------------
pdf("./output/ROC_of_model2_validation.pdf",height = 4,width = 5)
library(pROC)
