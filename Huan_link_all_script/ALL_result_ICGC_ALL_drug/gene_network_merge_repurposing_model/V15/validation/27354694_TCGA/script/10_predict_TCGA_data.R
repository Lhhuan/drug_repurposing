...
library(dplyr)
library(rminer)
setwd("/f/mulinlab/huan/ALL_result_ICGC_ALL_drug/gene_network_merge_repurposing_model/V13/validation/27354694_TCGA/script/")
#---------------训练集
dit<-"/f/mulinlab/huan/ALL_result_ICGC_ALL_drug/gene_network_merge_repurposing_model/V13/test_data/"
...
# pre <- svm(drug_repurposing ~.,data=org1)
# pre <- svm(x=org1,y=org$drug_repurposing ,type = 'nu-regression', kernel = 'radial', gamma=0.01, cost =100)
pre <- svm(x=org1,y=org$drug_repurposing ,type = 'nu-regression', kernel = 'radial')
summary(pre)
summary(org1)
summary(huan1)
#predict函数可以获得模型的预测值。这里预测所需的模型对象为pre，预测对象newdata为测试集,预测所需类型type选择response,对响应变量的区间进行调整
predict. <- predict(pre,huan1)
# predict. <- predict.glm(pre,type='response',newdata=huan1)
#按照预测值为1的概率，>0.5的返回1，其余返回0
predict =ifelse(predict.>0.506,1,0)
#把预测的具体值记录下来
...
setwd("/f/mulinlab/huan/ALL_result_ICGC_ALL_drug/gene_network_merge_repurposing_model/V13/validation/27354694_TCGA/output/")
