...
#需要预测的数据
huan<-read.table("./output/12_mark_positive_rwr.txt",header = T,sep = "\t") %>% as.data.frame()
...
huan_drug_cancer<-huan%>%dplyr::select(Drug_chembl_id_Drug_claim_primary_name,cancer_oncotree_id,cancer_oncotree_id_type,rwr_type)
...
predict =ifelse(predict.>0.366,1,0)
#把预测的具体值记录下来
...
setwd("/f/mulinlab/huan/ALL_result_ICGC_ALL_drug/gene_network_merge_repurposing_model/V2/validation/gdkb_cgi_oncokb_mtctscan/output/")
write.table(final_huan,"13_positive_sample_repurposing.txt",row.names = F, col.names = T,quote =F,sep="\t")#把表存下来
#------------------------------------------------
