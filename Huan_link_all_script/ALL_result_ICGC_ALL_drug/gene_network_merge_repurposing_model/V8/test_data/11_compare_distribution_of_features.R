...
library(readxl)
library(dplyr)
setwd("/f/mulinlab/huan/ALL_result_ICGC_ALL_drug/gene_network_merge_repurposing_model/V6/validation/27354694_TCGA/script/")
#---------------训练集
dit<-"/f/mulinlab/huan/ALL_result_ICGC_ALL_drug/gene_network_merge_repurposing_model/V6/test_data/"
...
huan_drug_cancer<-huan%>%dplyr::select(Drug_chembl_id_Drug_claim_primary_name,drug_in_paper,oncotree_id,oncotree_id_type
                                       ,paper_sample_name)
...
huan1$max_mutation_map_to_gene_level_score <-huan$max_mutation_map_to_gene_level_score

#----------------------------------------------对huan和train （org）加标签及合并。
huan1$class <-c("test data")
org1$class <-c("training data")
all_data<-bind_rows(huan1,org1)

#-------------------------------------画boxplot
fig <-all_data

convert <- function(fig) {
  lapply(1:c(ncol(fig)-1), function(i) { #从第1列开始循环,到ncol(fig)-1 结束
    fig2 <- fig[, c(i,19)] #d表的第i列和第19列
    fig2$type = colnames(fig2)[1] #d2的第二列是d2的type列
    colnames(fig2) = c("riskScore", "category", "type") #重命名d2列
    return(fig2)
  }) %>% do.call('rbind', .) #将上面的每次循环的表bind到一起。
}

ff <- convert(fig)# %>% filter(riskScore <10)

pdf("../figure/compare_distrbution_of_features_in_test_and_train.pdf",height = 5,width = 7) #把图片存下来

p1<-ggplot(ff, aes(type, riskScore, fill=factor(category))) + geom_boxplot(outlier.colour = NA)
p1<-p1+coord_cartesian(ylim = c(-5,15)) 
p1<-p1+theme(axis.text.x = element_text(size = 8,angle = 90,color = "black"))
p1<-p1+labs(fill= "Data type")
p1<-p1+xlab("Feature")+ylab("")
p1
dev.off()


# 
# ggplot(ff, aes(type, riskScore, group=factor(category), fill=factor(category))) +  #fill=type
...
# setwd("/f/mulinlab/huan/ALL_result_ICGC_ALL_drug/gene_network_merge_repurposing_model/V2/validation/27354694_TCGA/output/")
# write.table(final_huan,"10_prediction_logistic_regression.txt",row.names = F, col.names = T,quote =F,sep="\t")#把表存下来
# #------------------------------------------------
