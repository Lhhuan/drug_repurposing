#将drug 的prediction value 分成top 0_0.1, 0.1_0.2,0.2_0.8,0.8_0.9,0.9_1.0共7组做比较,分别得文件./output/17_pandrug_top_${top}_${buttom}.txt
#!/usr/bin/perl
use warnings;
use strict; 
use utf8;
...
my @pro = (0, 0.1, 0.2, 0.8, 0.9, 1);

for (my $i=1;$i<6;$i++){  #此时进行赋值
    ...
    system "sed -n '$true_top_line,${buttom_line}p' $f1 >./output/17_pandrug_top_${proportion}.txt";
    #---------------------------------------------------------------------------
    my $f2 = "./output/17_pandrug_top_${proportion}.txt";
    open my $I2, '<', $f2 or die "$0 : failed to open input file '$f2' : $!\n";
    my $fo1 = "./output/17_pandrug_top_${proportion}_final.txt";
