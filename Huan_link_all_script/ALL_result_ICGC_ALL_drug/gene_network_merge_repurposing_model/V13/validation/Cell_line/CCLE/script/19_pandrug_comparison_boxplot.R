...
org1<-read.table("./output/18_pandrug_top_0_0.4_final.txt",header = T,sep = "\t") %>% as.data.frame()
org2<-read.table("./output/18_pandrug_top_0.4_0.6_final.txt",header = T,sep = "\t") %>% as.data.frame()
org3<-read.table("./output/18_pandrug_top_0.6_1_final.txt",header = T,sep = "\t") %>% as.data.frame()
...
pdf("19_CCLE_top_0.4_0.6_1_group_comparison_p.pdf",height = 4,width = 5) #把图片存下来
...
my_comparisons <- list(c("0_0.4","0.4_0.6"), c("0_0.4","0.6_1"),c("0.4_0.6","0.6_1") ) 
p+coord_cartesian(ylim = c(12.5,17.5))+stat_compare_means(comparisons = my_comparisons,method = "wilcox.test", 
                                                         tip.length = 0.01,label.y=c(16.5,16.8,17.3))
##组内进行比较，指定p bar的长度并指定坐标轴p值标的位置
dev.off()

#----------------violin
pdf("19_CCLE_top_0.4_0.6_1_group_comparison_AUC_violin.pdf",height = 4,width = 5) #把图片存下来
