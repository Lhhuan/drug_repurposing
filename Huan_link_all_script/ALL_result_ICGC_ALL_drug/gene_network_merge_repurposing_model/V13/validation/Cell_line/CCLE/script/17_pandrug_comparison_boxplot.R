...
org1<-read.table("./output/16_pandrug_top_0_0.3_final.txt",header = T,sep = "\t") %>% as.data.frame()
org2<-read.table("./output/16_pandrug_top_0.3_0.7_final.txt",header = T,sep = "\t") %>% as.data.frame()
org3<-read.table("./output/16_pandrug_top_0.7_1_final.txt",header = T,sep = "\t") %>% as.data.frame()
...
pdf("17_CCLE_top_0.3_0.7_1_group_comparison_p.pdf",height = 4,width = 5) #把图片存下来
...
#----------------violin
pdf("17_CCLE_top_0.3_0.7_1_group_comparison_AUC_violin.pdf",height = 4,width = 5) #把图片存下来
