...
iris_model <- svm(x=org1,y=org$drug_repurposing ,type = 'nu-regression', 
                  kernel = 'radial')
iris_pred <- predict(object = iris_model, newdata = org1)
iris_Freq <- table(org$drug_repurposing, iris_pred)
iris_accuracy <- sum(diag(iris_Freq))/sum(iris_Freq)
iris_accuracy


org1$drug_repurposing <-org$drug_repurposing
tune.svm(drug_repurposing ~., data =org1, gamma = 10^(-100:-1), cost = 10^(0:3))
