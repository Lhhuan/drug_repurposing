...
main_cancer<-read_tsv(file.path(dit, "network_based_main_cancer_proportion_test.txt")) %>% as.data.frame()
detail_cancer<-read_tsv(file.path(dit, "network_based_detail_cancer_proportion_test.txt")) %>% as.data.frame()
...
detail_than_0<-detail%>%filter(final_percentage>0)%>%arrange(final_percentage)
...
setwd("/f/mulinlab/huan/figure/figures/")
pdf("Network_based_drug_main_cancer_pairs_with_genetic_associations_percentage_large_than_0.pdf",height = 3.5,width = 5) #把图片存下来
