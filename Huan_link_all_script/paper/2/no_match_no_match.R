...
#抓出来的文章
setwd ("~/paper/2/")
match_title <- read.table("~/paper/2/2_variant_title_to_PMID.txt",header = T)%>%unique()


#以前抓的文章，第一次和第的整合的文章(文件夹0和1里面的数据）
setwd ("~/paper/")
before_title <-read.table("12_match.txt",header = T)%>%unique()%>%dplyr::select(title) #读之前抓的列表
before_title1 <- before_title[rowSums(is.na(before_title))==0,]%>%as.data.frame()

t <- read_xlsx("~/paper/2_jou.xlsx",col_names = F)  #读杂志列表
...
setwd ("~/paper/2")
write.table(no_match1,'2_no_match_variant_title_to_PMID.txt',row.names = F, col.names = T)
write.table(true_unique_match,'2_ture_match_variant_title_to_PMID.txt',row.names = F, col.names = T)
