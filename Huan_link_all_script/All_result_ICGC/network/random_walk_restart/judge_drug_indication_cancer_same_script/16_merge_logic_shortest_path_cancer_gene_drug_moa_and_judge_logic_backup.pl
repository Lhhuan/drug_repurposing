##把网络最短路径的文件10_start_end_logical.txt及15_network_based_ICGC_somatic_repo_may_success.txt merge在一起，得文件16_merge_logic_shortest_path_cancer_gene_drug_moa.txt（因为测试过中间文件很大，所以就不输出了，直接对其判断逻辑）
...
# system "sort 15_network_based_ICGC_somatic_repo_may_success.txt | uniq >15_network_based_ICGC_somatic_repo_may_success_unique.txt" ; 
...
$header = "$header\tindication_OncoTree_IDs_detail\tindication_OncoTree_main_term\tindication_OncoTree_main_ID\tdrug_target_score\tthe_final_logic";
print $O1 "$header\n";
print $O2 "$header\n";
print $O3 "$header\n";
# print $O4 "$header\n";
...
while(<$I1>)
{
    ...
    unless(/^drug_name_network/){
        ...
        my @start_ids = split/\,/,$start_id1;
        foreach my $start_id(@start_ids){ #对每个start 进行循环，于end 组合寻找符合要求的最短路径
