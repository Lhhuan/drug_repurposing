...
my $fo1 ="./original_network_num.txt"; #将normal_network.txt的gene换成id。#header = "id1\tid2\tweight\n" #这里的weight是两个节点之间的probility(可能性)转化而来。
...
while(<$I1>)
{
    ...
    my @f= split /\t/;
     unless(/^Gene1/){
         ...
         my $gene2 = $f[1];
         $gene2 =~s/\s+/-/g;
         my $probility = $f[4];#probility越大，weight 越小。
...
foreach my $ID (sort keys %hash1){
    if (exists $hash2{$ID}){
        ...
        foreach my $v(@v){
            ...
            my @f= split/\t/,$group1;
            my $gene2 = $f[1];
            my $vg = join ("\t", @f[0,2]); #$f[0]是原来表格中的gene1转换后的id，也就是$s,
            push @{$hash3{$gene2}},$vg;
            #print STDERR "$group1";
        }
    } 
