...
setwd("~/All_result/network/the_shortest_path")
#org<-read.table("/f/mulinlab/huan/All_result/network/network_gene_num.txt",header = T)
org<-read.table("/f/mulinlab/huan/All_result_ICGC/network/network_gene_num.txt",header = T)
spe<-data.frame(org$id)#把所有的顶点数读进来。
input<-read.table("normal_network_num.txt",header = F)  #把文件读进来并转为data.frame
df <- data.frame(from=c(input$V1), to=c(input$V2), weight=c(input$V3))#把读进来的文件转化成图的start,end,weight
g <- graph.data.frame(df,directed=TRUE,vertices=spe) #把读进来的文件转化成图


pairs <-read.table("test_start_end.txt",header = T)%>%as.data.frame()#把要看最短路径start和end的列表读进来。
...
write.table(rs,"the_shortest_path.txt",row.names = F, col.names = T,quote =F,sep="\t")#把表存下来
