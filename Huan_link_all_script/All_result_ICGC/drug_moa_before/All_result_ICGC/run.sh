...
#得文件13_ICGC_cancer_gene_drug_information.txt,得没有药物基因的somatic mutation得13_ICGC_no_drug_somatic_path_gene_role.txt,得co_gene是13_drug_cancer_co_ensg_count.txt 
...
perl 15_merge_cancer_gene_drug_indication_oncotree.pl ##把13_ICGC_cancer_gene_drug_information.txt和huan_mapin_do_hpo_oncotree_main1.txt,merge在一起，得到文件15_cancer_gene_drug_indication_oncotree.txt
...
perl 21_merge_drug_info_do_hpo_oncotree.pl #把huan_target_drug_indication_final_symbol_drug-class.txt和huan_mapin_do_hpo_oncotree_main1.txt merge 在一起，得21_all_drug_infos.txt
...
cp /f/mulinlab/sinan/drug_target_score_v3.txt  ./DGIDB_drug_target_score1.txt #此文件为翟思南和吴承蔚从DGIDB手动扣下来的drug target score #
#从翟思南哪里copy的文件有错误，由DGIDB_drug_target_score1.txt 修正得DGIDB_drug_target_score.txt
perl normal_DGIDB_drug_target_score.pl  #把DGIDB_drug_target_score.txt的name 进行normal ,(因为之前给sinan的文件中 repurpose hub没有严格按照有chembl的chembl填充，现在之前的文件已经修改好，
...
perl extract_drug_primary_name_and_merge_drug_target_gene_score_ensgid_entrezid.pl #在huan_target_drug_indication_final_symbol.txt中提取normal_DGIDB_drug_target_score.txt 的Drug_chembl_id|Drug_claim_primary_name所对应的Drug_claim_primary_name
