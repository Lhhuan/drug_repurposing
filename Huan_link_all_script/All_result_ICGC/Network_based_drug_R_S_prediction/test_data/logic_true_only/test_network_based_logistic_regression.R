...
library(readxl)
library(dplyr)
setwd("/f/mulinlab/huan/All_result_ICGC/Network_based_drug_R_S_prediction/test_data/logic_true_only/")
...
for(i in 1:5){
  ...
  #----------------------------------------------
  true_value1 =fold_test[,7]
  #--------------------------把所有的被预测的测试数据的结果放到一个表里
  tmp<-data.frame(true_value1= fold_test[,7],predict_value1=fold_predict)
  rs <- bind_rows(rs,tmp)
  #---------------------------
  fold_error = fold_test[,8]-fold_test[,7]
  ...
  fold_error2 = fold_train[,8]-fold_train[,7]
