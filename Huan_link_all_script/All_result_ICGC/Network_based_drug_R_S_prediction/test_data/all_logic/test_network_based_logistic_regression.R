...
library(readxl)
library(dplyr)
setwd("/f/mulinlab/huan/All_result_ICGC/Network_based_drug_R_S_prediction/test_data/all_logic/")
...
org1<-org %>% dplyr::select(average_drug_score,averge_gene_mutation_frequency,average_gene_CADD_score,average_mutation_map_to_gene_level_score,average_path_length
                            ,min_rwr_normal_P_value,logic_true_value,drug_repurposing)
#将org1数据分成随机十等分
...
for(i in 1:5){
  ...
  #----------------------------------------------
  true_value1 =fold_test[,8]
  #--------------------------把所有的被预测的测试数据的结果放到一个表里
  tmp<-data.frame(true_value1= fold_test[,8],predict_value1=fold_predict)
  rs <- bind_rows(rs,tmp)
  #---------------------------
  fold_error = fold_test[,9]-fold_test[,8]
  ...
  fold_error2 = fold_train[,9]-fold_train[,8]
