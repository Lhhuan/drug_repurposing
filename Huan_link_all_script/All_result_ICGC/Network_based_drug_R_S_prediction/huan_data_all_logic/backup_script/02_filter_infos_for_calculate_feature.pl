...
#提取"/f/mulinlab/huan/All_result_ICGC/network/random_walk_restart/16_judge_the_shortest_drug_target_cancer_gene_logic_true.txt" 中不部分列，用来计算feature，得./output/02_filter_infos_for_calculate_feature.txt
#!/usr/bin/perl
use warnings;
use strict; 
use utf8;
...
$header = "$header\tcancer_ENSG\toncotree_ID_main_tissue\tMap_to_gene_level\tmap_to_gene_level_score";
print $O1 "$header\n";
...
while(<$I1>)
{
    ...
    unless(/^the_shortest_path/){
       ...
       $output1 = "$output1\t$cancer_ENSG\t$oncotree_ID_main_tissue\t$map_to_gene_level";
