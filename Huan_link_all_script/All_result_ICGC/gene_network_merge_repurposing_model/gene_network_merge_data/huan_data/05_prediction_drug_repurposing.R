...
#需要预测的数据
huan<-read.table("./output/04_calculate_features_for_logistic_regression.txt",header = T,sep = "\t") %>% as.data.frame()
huan1<-huan%>%dplyr::select(average_effective_drug_target_score,averge_mutation_frequency,averge_mutation_pathogenicity,averge_mutation_map_to_gene_level_score
                                      ,average_the_shortest_path_length,min_rwr_normal_P_value,median_rwr_normal_P_value,cancer_gene_exact_match_drug_target_ratio,average_del_svscore
...
write.table(huan1,"05_logistic_regression_prediction_potential_drug_repurposing_data.txt",row.names = F, col.names = T,quote =F,sep="\t")#把表存下来
#------------------------------------------------
