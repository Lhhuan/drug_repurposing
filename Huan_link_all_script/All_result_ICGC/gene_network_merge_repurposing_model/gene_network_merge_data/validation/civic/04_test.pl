...
foreach my $drug_cancer_pairs(sort keys %hash2){
    ...
    foreach my $info(@infos){
        ...
        if(exists $hash1{$drug_cancer_pairs}){ 
            if ($variant =~/FUSION|DELETION|COPY NUMBER VARIATION|AMPLIFICATION/i){ #把有点突变的FUSION|DUP|DELETION|COPY NUMBER VARIATION|AMPLIFICATION纳入进来，其他的FUSION|DUP|DELETION和 over expression等
                # if($variant =~/.*([A-Z]\d+[A-Z]).*/){
                unless($variant=~/[A-Z]\d+[A-Z]/){
