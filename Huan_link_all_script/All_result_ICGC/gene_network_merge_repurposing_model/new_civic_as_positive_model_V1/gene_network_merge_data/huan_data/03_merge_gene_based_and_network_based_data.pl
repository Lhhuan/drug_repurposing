...
my $fo1 = "./output/03_merge_gene_based_and_network_based_data.txt";
...
$header = "$header\tcancer_ENSG\toncotree_ID_detail\toncotree_ID_main_tissue\tthe_final_logic\tMap_to_gene_level\tmap_to_gene_level_score\tdata_source";
print $O1 "$header\n";
...
while(<$I1>)
{
    ...
    unless (/^Drug_chembl_id/){
        ...
        my $map_to_gene_level_score = $f[20];
        my $output = join("\t",$f[0],@f[2..13],$f[15],$f[17],@f[18..20]);
...
while(<$I2>)
{
    ...
    unless (/^CADD_MEANPHRED/){
        ...
        my $output1 = "$output\t$cancer_specific_affected_donors\t$original_cancer_ID\t$CADD_MEANPHRED\t$cancer_ENSG\t$oncotree_ID_detail\t$cancer_oncotree_main_id\t$the_final_logic\t$map_to_gene_level";
...
system "sort ./output/03_merge_gene_based_and_network_based_data.txt | uniq >./output/03_unique_merge_gene_based_and_network_based_data.txt" ; #暂时注释