...
library(readxl)
library(dplyr)
setwd("/f/mulinlab/huan/All_result_ICGC/gene_network_merge_repurposing_model/new_civic_as_positive_model/gene_network_merge_data/huan_data/")
#---------------训练集
#org<-read.table("/f/mulinlab/huan/All_result_ICGC/gene_network_merge_repurposing_model/new_civic_as_positive_model/gene_network_merge_data/test_data/output/09_filter_test_data_for_logistic_regression.txt",header = T,sep = "\t") %>% as.data.frame()
org<-read.table("/f/mulinlab/huan/All_result_ICGC/gene_network_merge_repurposing_model/new_civic_as_positive_model/gene_network_merge_data/test_data/output/04_filter_features_cancer_phase_than0.txt",header = T,sep = "\t") %>% as.data.frame()
...
#需要预测的数据
huan<-read.table("./output/07_filter_icgc_data_features_for_logistic_regression.txt",header = T,sep = "\t") %>% as.data.frame()
...
predict =ifelse(predict.>0.253,1,0)
#把预测的具体值记录下来
...
setwd("/f/mulinlab/huan/All_result_ICGC/gene_network_merge_repurposing_model/new_civic_as_positive_model/gene_network_merge_data/huan_data/output/")
