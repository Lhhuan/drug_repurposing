#将../output/11_merge_prediction_repurposing_drug_sensitive_info_sample.txt prediction value 分成top 0-0.1, 0.1-0.9, 0.9-1 共3组做比较,
...
for (my $i=1;$i<4;$i++){  #此时进行赋值
    ...
    system "sed -n '$true_top_line,${buttom_line}p' $f1 >../output/12_pandrug_top_${proportion}.txt";
    #---------------------------------------------------------------------------
    my $f2 = "../output/12_pandrug_top_${proportion}.txt";
    open my $I2, '<', $f2 or die "$0 : failed to open input file '$f2' : $!\n";
    my $fo1 = "../output/12_pandrug_top_${proportion}_final.txt";
    open my $O1, '>', $fo1 or die "$0 : failed to open output file '$fo1' : $!\n";

    my $header = "Drug_chembl_id_Drug_claim_primary_name\toncotree_id\toncotree_id_type\tpaper_sample_name\tpredict_value\tCompound\tIC50\tActArea";
    if ($i>1){
