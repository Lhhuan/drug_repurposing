#用../data/CCLE_DepMap_18q3_maf_20180718.txt 和../data/CCLE_copynumber_2013-12-03.seg.txt， ../data/CCLE_translocations_SvABA_20181221.txt 提取../output/021_merge_sample_oncotree_chembl.txt中
...
while(<$I1>)
{
    ...
    unless (/^CCLE_name/){
       ...
       # my $k = "$CCLE_name\t$Cell_line_primary_name";
        my $k = $CCLE_name;
        my $oncotree_ID_drug = join ("\t",@f[2..6],$f[-1]);
        $hash1{$k} =$oncotree_ID_drug;
        my $oncotree_id = join("\t",@f[2..5]);
        $hash2{$k} =$oncotree_id;
    }
}
...
while(<$I2>)  #snv
{
    ...
    unless (/^Hugo_Symbol/){
        ...
        my $CCLE_name = $Tumor_Sample_Barcode;
