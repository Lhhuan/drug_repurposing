#用../output/08_merge_all_sv.txt 和../output/05_filter_snv_in_huan.txt 计算features.得../output/09_calculate_features_for_logistic_regression.txt
#!/usr/bin/perl
use warnings;
use strict; 
use utf8;
...
my $f1 = "../output/05_filter_snv_in_huan.txt";
open my $I1, '<', $f1 or die "$0 : failed to open input file '$f1' : $!\n";
my $f2 = "../output/08_merge_all_sv.txt";
open my $I2, '<', $f2 or die "$0 : failed to open input file '$f2' : $!\n";
...
while(<$I2>)
{
    ...
    unless (/^#CHROM/){
        ...
        my $sample = $f[0];
        my $SVSCORETOP10 = $f[1];
        my $sv_or_cnv_type = $f[2];
        my $sv_or_cnv_id  =$f[3];
        my $oncotree_ID = $f[4];
        my $oncotree_ID_type = $f[5];        
        my $k = "$sample\t$oncotree_ID\t$oncotree_ID_type";
...
foreach my $k (sort keys %hash1){
    ...
    my $k_sv = "$paper_sample_name\t$oncotree_id\t$oncotree_id_type";
    my $project = $k_sv;
