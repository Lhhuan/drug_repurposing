# #将../output/11_merge_prediction_repurposing_drug_sensitive_info_sample.txt prediction value 分成top 0-0.5, 0.5-1 共2组做比较,
...
for (my $i=1;$i<3;$i++){  #此时进行赋值
    ...
    system "sed -n '$true_top_line,${buttom_line}p' $f1 >../output/20_pandrug_top_${proportion}.txt";
    #---------------------------------------------------------------------------
    my $f2 = "../output/20_pandrug_top_${proportion}.txt";
    open my $I2, '<', $f2 or die "$0 : failed to open input file '$f2' : $!\n";
    my $fo1 = "../output/20_pandrug_top_${proportion}_final.txt";
