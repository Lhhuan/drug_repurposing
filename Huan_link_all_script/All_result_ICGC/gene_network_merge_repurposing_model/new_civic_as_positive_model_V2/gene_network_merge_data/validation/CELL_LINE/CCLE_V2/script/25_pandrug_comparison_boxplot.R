...
org1<-read.table("./output/24_pandrug_top_0_0.7_final.txt",header = T,sep = "\t") %>% as.data.frame()
org2<-read.table("./output/24_pandrug_top_0.7_1_final.txt",header = T,sep = "\t") %>% as.data.frame()
...
pdf("25_CCLE_top_0.7_1_group_comparison_p.pdf",height = 4,width = 5) #把图片存下来
...
my_comparisons <- list(c("0_0.7","0.7_1")) 
...
#----------------violin
pdf("25_CCLE_top_0.7_1_group_comparison_AUC_violin.pdf",height = 4,width = 5) #把图片存下来
