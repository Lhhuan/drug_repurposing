...
org1<-read.table("./output/20_pandrug_top_0_0.5_final.txt",header = T,sep = "\t") %>% as.data.frame()
org2<-read.table("./output/20_pandrug_top_0.5_1_final.txt",header = T,sep = "\t") %>% as.data.frame()
...
pdf("21_CCLE_top_0.5_1_group_comparison_p.pdf",height = 4,width = 5) #把图片存下来
...
#----------------violin
pdf("21_CCLE_top_0.5_1_group_comparison_AUC_violin.pdf",height = 4,width = 5) #把图片存下来
