#取不同的./output/07_sorted_merge_negative_positive.txt top ratio，看number_of_positive，得文件./output/09_number_of_positive_in_multi_top_ratio.txt
...
my $f2 ="./output/07_sorted_merge_negative_positive.txt";#
...
for (my $i=1;$i<11;$i++){ #对文件进行处理，把所有未定义的空格等都替换成NONE
    ...
    my $p_ratio= $positive_num / 316; #占正样本的比例
    $p_ratio =$p_ratio *100;
