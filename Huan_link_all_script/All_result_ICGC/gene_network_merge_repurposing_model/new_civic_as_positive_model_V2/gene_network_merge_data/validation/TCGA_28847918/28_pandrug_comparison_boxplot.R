...
org1<-read.table("./output/27_pandrug_top_0_0.3_final.txt",header = T,sep = "\t") %>% as.data.frame()
org2<-read.table("./output/27_pandrug_top_0.3_0.7_final.txt",header = T,sep = "\t") %>% as.data.frame()
org3<-read.table("./output/27_pandrug_top_0.7_1_final.txt",header = T,sep = "\t") %>% as.data.frame()
...
pdf("28_pandrug_top_0.3_0.7_1_group_comparison_p.pdf",height = 4,width = 5) #把图片存下来
...
my_comparisons <- list(c("0_0.3","0.3_0.7"), c("0_0.3","0.7_1"),c("0.3_0.7","0.7_1") ) 
p+coord_cartesian(ylim = c(-0.5,0.85))+stat_compare_means(comparisons = my_comparisons,method = "wilcox.test", 
                                                           tip.length = 0.01,label.y=c(0.63,0.73,0.85))
##组内进行比较，指定p bar的长度并指定坐标轴p值标的位置
dev.off()

#----------------violin
pdf("28_pandrug_top_0.3_0.7_1_group_comparison_violin.pdf",height = 4,width = 5) #把图片存下来
