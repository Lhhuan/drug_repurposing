...
#需要预测的数据
huan<-read.table("./output/02_calculate_features_for_logistic_regression.txt",header = T,sep = "\t") %>% as.data.frame()
#huan_drug_cancer<-huan%>%dplyr::select(Drug_chembl_id_Drug_claim_primary_name,cancer_oncotree_id,cancer_oncotree_id_type)
huan_drug_cancer<-huan%>%dplyr::select(Drug_chembl_id_Drug_claim_primary_name,oncotree_id,oncotree_id_type,paper_sample_name)
...
write.table(final_huan1,"./output/03_prediction_logistic_regression.txt",row.names = F, col.names = T,quote =F,sep="\t")#把表存下来
#------------------------------------------------
