#用"/f/mulinlab/huan/All_result_ICGC/ICGC_occurthan1_snv_indel_project_oncotree_normalized.txt"中的detail id 和 main id
...
my $f2 ="./output/11_drug_unique_status_infos.txt";
...
$header = "$header\tcancer_oncotree_id_type\tpredict\tpredict_value\tDrug_indication_Indication_class\tindication_OncoTree_detail_ID\tindication_OncoTree_main_ID\tMax_phase\tFirst_approval";
print $O1 "$header\n";
...
{
    chomp;
    unless(/^Drug_chembl_id_Drug_claim_primary_name/){  #原来的$cancer_oncotree_id变成了$indication_OncoTree_detail_ID
        my @f= split /\t/;
