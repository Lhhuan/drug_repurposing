...
main_cancer<-read_tsv(file.path(dit, "gene_based_logic_true_drug_count_in_main_cancer_gene_top.txt")) %>% as.data.frame()
detail_cancer<-read_tsv(file.path(dit, "gene_based_logic_true_drug_count_in_detail_cancer_gene_top.txt")) %>% as.data.frame()
...
for(i in 1:le_m ){
  aa<- main_cancer[i,3]
  bb<- main_cancer[i,4]
  p_t_main<-prop.test(x = c(aa, 11214), n = c(bb, 130856))
...
for(i in 1:le_d ){
 ...
 # p_t_d<-prop.test(aa, bb)
  p_t_d<-prop.test(x = c(aa, 15311), n = c(bb, 303348))
  p_d<-data.frame(p_t_d[3],p_t_d$conf.int[1],p_t_d$conf.int[2]) 
  rs_d <- bind_rows(rs_d,p_d)
}
detail_cancer<-cbind(detail_cancer,rs_d)
write.table(main_cancer,"gene_based_main_cancer_proportion_test_top.txt",row.names = F, col.names = T,quote =F,sep="\t")#把表存下来
write.table(detail_cancer,"gene_based_detail_cancer_proportion_test_top.txt",row.names = F, col.names = T,quote =F,sep="\t")#把表存下来

# 
# tmp <- main_cancer
# tmp <- tmp %>% mutate(perc=str_replace(percentage, "%", "")) %>% mutate(perc=as.numeric(perc))
# ova <- tmp %>% filter(Cancer_term=="Overall") %>% pull(perc)
# 
# tmp2 <- tmp %>% mutate(perc2=perc/ova)
