#把./output/02_unique_filter_network_based_infos.txt 取特定列和
...
my $f1 = "./output/02_unique_filter_network_based_infos.txt";
my $f2 = "./output/01_filter_gene_based_drug_target_score_cancer_mutation_pathogenicity.txt";
my $fo1 = "./output/merge_gene_based_and_network_based_data_for_figure.txt";
...
$header = "$header\tcancer_ENSG\toncotree_term_detail\toncotree_ID_detail\toncotree_term_main_tissue\toncotree_ID_main_tissue\tthe_final_logic\tMap_to_gene_level\tproject\tmap_to_gene_level_score\tdata_source";
print $O1 "$header\n";
...
while(<$I1>)
{
    ...
    unless (/^Drug_chembl_id/){
        ...
        my $output = join("\t",$f[0],@f[2..13],@f[14..17],@f[18..21]);
...
while(<$I2>)
{
    ...
    unless (/^CADD_MEANPHRED/){
        ...
        my $cancer_oncotree_main_term = $f[12];
        ...
        my $output1 = "$output\t$cancer_specific_affected_donors\t$original_cancer_ID\t$CADD_MEANPHRED\t$cancer_ENSG\t$oncotree_term_detail\t$oncotree_ID_detail\t$cancer_oncotree_main_term\t$cancer_oncotree_main_id\t$the_final_logic\t$map_to_gene_level\t$project";
...
system "sort ./output/merge_gene_based_and_network_based_data_for_figure.txt | uniq >./output/unique_merge_gene_based_and_network_based_data_for_figure.txt" ; #暂时注释