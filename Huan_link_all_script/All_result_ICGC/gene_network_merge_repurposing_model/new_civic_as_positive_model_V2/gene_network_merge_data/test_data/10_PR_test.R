...
label <- rs$true_value1
decision <- rs$predict_value1
...
for(i in 0:ngrids){
  ...
  A[i] <- sum((pred_label == label)*1)/nrow(rs)
}

plot(R, P, col = "#2f89fc",lwd=3, type="l",xlab="Recall",ylab="Precision", main="PR Curve")
grid(5, 5, lwd = 1)
