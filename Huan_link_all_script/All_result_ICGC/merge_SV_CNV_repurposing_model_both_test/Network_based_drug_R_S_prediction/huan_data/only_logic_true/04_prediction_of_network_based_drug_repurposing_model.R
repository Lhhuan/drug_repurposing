...
library(readxl)
library(dplyr)
setwd("/f/mulinlab/huan/All_result_ICGC/merge_SV_CNV_repurposing_model_both_test/Network_based_drug_R_S_prediction/huan_data/only_logic_true/")
#---------------训练集
org<-read.table("/f/mulinlab/huan/All_result_ICGC/merge_SV_CNV_repurposing_model_both_test/Network_based_drug_R_S_prediction/test_data/logic_true_only/output/04_final_data_for_logistic_regression.txt",header = T,sep = "\t") %>% as.data.frame()
...
predict =ifelse(predict.>0.419,1,0)
#把预测的具体值记录下来
...
rs <- data.frame()#构造一个表
setwd("/f/mulinlab/huan/All_result_ICGC/merge_SV_CNV_repurposing_model_both_test/Network_based_drug_R_S_prediction/huan_data/only_logic_true/output/")
