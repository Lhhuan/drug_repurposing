...
#需要预测的数据
test_huan<-read.table("/f/mulinlab/huan/All_result_ICGC/merge_SV_CNV_repurposing_model_both_test/Gene-based_drug_R_or_S_optimization/huan_data/output/023_calculate_for_gene_based_repo_logistic_regression_data_final.txt",header = T,sep = "\t") %>% as.data.frame()
...
predict =ifelse(predict.>0.367,1,0)
#把预测的具体值记录下来
predict_value = predict.
#数据中加入预测值一列
test_huan1$predict = predict
test_huan1$predict_value = predict_value
setwd("/f/mulinlab/huan/All_result_ICGC/merge_SV_CNV_repurposing_model_both_test/Gene-based_drug_R_or_S_optimization/huan_data/output/")
