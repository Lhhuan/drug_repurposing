#用unique_Drug_claim_primary_name_Drug_chembl_id.txt 中Drug_claim_primary_name从huan_target_drug_indication_final_symbol_drug-class.txt提取其他的信息
...
my $f1 ="./unique_Drug_claim_primary_name_Drug_chembl_id.txt";
    ...
    unless(/^Drug_chembl_id/){
        ...
        if (exists $hash1{$Drug_claim_primary_name}){
            ...
            foreach my $v (@vs){
                ...
                my $out = "$Gene_symbol\t$Entrez_id\t$Interaction_types\t$Drug_claim_primary_name";
                my $out2 = join("\t",@f[6..17]);
                my $output = "$out\t$out2";
