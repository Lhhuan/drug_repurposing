...
my $fo2 ="./oncokb_cosmic_gene_role.txt";#oncokb 和cosmic中都有的具有oncorole 的基因。
my $fo3 ="./oncokb_single_c_gene_role.txt";#只在oncokb中存在，不在cosmic中存在的具有oncorole 的基因
...
print "Gene\tRole\n";
select $O2;
print "Gene\tRole_cosmic\tRole_oncokb\n";
select $O3;
print "Gene\tRole\n";
...
foreach my $ID (sort keys %hash3){
    if (exists $hash2{$ID}){
        my $v2= defined($hash2{$ID}) ? $hash2{$ID} : "NA"; #如果$hash2{$ID}定义了，就把$hash2{$ID}给$v2,如果$hash2{$ID}没定义，就把NA给$v2;类似于if ,else
        my $v3 = $hash3{$ID};
        print $O2 "$ID\t$v2\t$v3\n";
    } 
    else {
