...
library(readxl)
library(dplyr)
setwd("/f/mulinlab/huan/All_result_ICGC/merge_SV_CNV_repurposing_model/Network_based_drug_R_S_prediction/huan_data/only_logic_true/")
#---------------训练集
org<-read.table("/f/mulinlab/huan/All_result_ICGC/merge_SV_CNV_repurposing_model/Network_based_drug_R_S_prediction/test_data/logic_true_only/output/03_final_filter_repo_withdrwal_data_for_logistic_regression.txt",header = T,sep = "\t") %>% as.data.frame()
org1<-org %>% dplyr::select(average_drug_score,averge_gene_mutation_frequency,average_gene_CADD_score,average_mutation_map_to_gene_level_score,average_path_length
                            ,min_rwr_normal_P_value,averge_gene_num_in_del_hotspot,averge_gene_num_in_dup_hotspot,averge_gene_num_in_inv_hotspot,
                            averge_gene_num_in_tra_hotspot,averge_gene_num_in_cnv_hotspot,drug_repurposing)
#需要预测的数据
...
test_huan1<-test_huan%>%dplyr::select(average_drug_score,averge_gene_mutation_frequency,average_gene_CADD_score,average_mutation_map_to_gene_level_score,average_path_length
                                      ,min_rwr_normal_P_value,averge_gene_num_in_del_hotspot,averge_gene_num_in_dup_hotspot,averge_gene_num_in_inv_hotspot,
                                      averge_gene_num_in_tra_hotspot,averge_gene_num_in_cnv_hotspot)
#----------------------------------------------------------------------------------------------------
#生成logis模型，用glm函数
...
rs <- data.frame()#构造一个表
setwd("/f/mulinlab/huan/All_result_ICGC/merge_SV_CNV_repurposing_model/Network_based_drug_R_S_prediction/huan_data/only_logic_true/output/")
