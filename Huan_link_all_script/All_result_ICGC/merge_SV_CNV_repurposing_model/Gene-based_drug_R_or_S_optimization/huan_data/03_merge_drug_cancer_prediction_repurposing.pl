#因为prediction_repurposing.txt中没有drug cancer 的信息，所以用02_calculate_for_repo_logistic_regression_data.txt和prediction_repurposing.txt merge起来，得到03_drug_cancer_prediction_repurposing.txt
#并得到预测结果为repurposing 的文件03_drug_cancer_prediction_repurposing_true.txt
#!/usr/bin/perl
use warnings;
use strict; 
use utf8;
...
my $f2 = "./prediction_repurposing.txt";
my $fo1 = "./03_drug_cancer_prediction_repurposing.txt";
...
while(<$I1>)
{
    ...
    if(/^Drug/){
        ...
        my $k = "$average_drug_score\t$averge_gene_mutation_frequency\t$average_gene_CADD_score\t$average_mutation_map_to_gene_level_score";
        push @{$hash1{$k}},$v;
    }
}
...
my $f3 = "./03_drug_cancer_prediction_repurposing.txt";
my $fo2 = "./03_drug_cancer_prediction_repurposing_true.txt";
