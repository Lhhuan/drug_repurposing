#判断"./output/04_drug_cancer_gene_based_prediction_potential_drug_repurposing_indication.txt"中的indication和cancer是否相同，
...
my $f1 ="./output/04_drug_cancer_gene_based_prediction_potential_drug_repurposing_indication.txt";
open my $I1, '<', $f1 or die "$0 : failed to open input file '$f1' : $!\n";
my $fo1 ="./output/05_indication_and_cancer_same.txt"; 
open my $O1, '>', $fo1 or die "$0 : failed to open output file '$fo1' : $!\n";
my $fo2 ="./output/05_indication_and_cancer_differ.txt"; 
...
my $f2 ="./output/05_indication_and_cancer_differ.txt";
open my $I2, '<', $f2 or die "$0 : failed to open input file '$f2' : $!\n";
my $fo3 ="./output/05_indication_and_cancer_differ_info.txt"; 
    ...
    unless(/^drug/){
        ...
        if(exists $hash7{$k}){
            ...
            foreach my $drug_cancer_info (@drug_cancer_infos){
                ...
                my $output = join("\t",@f[0..9]); #不再输出indication
...
# foreach my $drug(sort keys %hash9){   #检查得重复的最大status 都是Launched
