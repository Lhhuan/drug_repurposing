...
my $f1 ="./uni_NofPmids3_disease_num.txt";
my $f2 = "./42HPMZB.id-id.mapin";
my $f3 = "../ID-term.txt";
my $fo1 = "./01_NofPmids3_disease_indication_do_term.txt";  
...
select $O1;
print "ID\tindication\tDO_ID\tDO_term\n";
# select $O2;
# print "ID\tindication\n";
...
while(<$I3>)
{
 ...
 foreach my $key1(sort keys %hash1){
       if(exists $hash2{$key1}){ 
           ...
           my $indication = $hash1{$key1};
            print $O1 "$key1\t$indication\tNA\tNA\n";
            # print $O1 "$v\n";
        }
 }
