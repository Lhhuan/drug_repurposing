...
pairs <-read.table("./08_uni_start_end_shortest15.txt",header = F)%>%as.data.frame()#把要看最短路径start和end的列表读进来。
rs <- data.frame()#构造一个表
...
write.table(rs,"09_the_shortest_path15.txt",row.names = F, col.names = T,quote =F,sep="\t")#把表存下来
