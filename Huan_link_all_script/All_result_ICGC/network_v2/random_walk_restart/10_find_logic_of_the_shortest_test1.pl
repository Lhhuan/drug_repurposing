#用"/f/mulinlab/huan/All_result_ICGC/network/the_shortest_path/normal_network_num.txt"为the_shortest_path.txt里面的路径寻找start和end的逻辑关系。
#!/usr/bin/perl
use warnings;
use strict; 
use utf8;
...
#     # if (abs($in-$act)<0.000001){ #此处判断$in==$act;
#     #     # my $s = abs($in-$act);
#     #     # print STDERR "$s\n";
#     #     print $O2 "$start_end\t-\n";
#     # }
#     # elsif($in-$act<0){ #此处判断$in == 0; $act==1;
#     #     print $O2 "$start_end\ta\n";
#     # }
#     # else{
#     #     print $O2 "$start_end\tin\n"; #此处判断$in == 1; $act==0;
