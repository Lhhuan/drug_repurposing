...
fun <- function(i){
  ...
  write.table(rs,"09_the_shortest_path_parallel.txt",row.names = F, col.names = T,quote =F,sep="\t")
}
#-------------------------------------
library(parallel)
cl.cores <- detectCores(logical = F) #检查核数
system.time({#返回计算所用的时间
  cl <- makeCluster(10)#初始化核心集群
  results <- parLapply(cl,1:10,fun)#并行程序
  res <- do.call('rbind',results)#整合结果
  stopCluster(cl)#关闭集群
})
